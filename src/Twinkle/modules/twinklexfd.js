...
(function twinklexfd() {
	...
	Twinkle.xfd.callbacks = {
				...
				switch (type) {
					...
					case 'batch': {
						...
						if (text.includes(commentText)) {
							...
							}的福瑞百科编者及时间：<br id="no-new-title">~~`.concat('~~')}`;
		...
		ffd: {
			main: (tagging_page) => {
				...
				wikiPage.setFollowRedirect(true);
				wikiPage.setCallbackParameters(params);
				wikiPage.load(Twinkle.xfd.callbacks.ffd.todaysList);
				// Notification to first contributor
				if (params.notify) {
					// Disallow warning yourself
					if (params.creator === mw.config.get('wgUserName')) {
		...
		addToLog: (params, initialContrib) => {
			...
			usl.initialText = window.wgULS(
				`这是该用户使用[[H:TW|Twinkle]]的提删模块做出的[[FWIKI:XFD|存废讨论]]提名列表。\n\n若您不再想保留此日志，请在[[${Twinkle.getPref(
					'configPage'
				)}|参数设置]]中关掉，并使用[[FWIKI:CSD#O1|CSD O1]]提交快速删除。`,
				`這是該使用者使用[[H:TW|Twinkle]]的提刪模塊做出的[[FWIKI:XFD|存廢討論]]提名列表。\n\n若您不再想保留此日誌，請在[[${Twinkle.getPref(
					'configPage'
				)}|偏好設定]]中關掉，並使用[[FWIKI:CSD#O1|CSD O1]]提交快速刪除。`
			);
			let xfdCatName;
			switch (params.xfdcat) {
				case 'delete':
	...
	Twinkle.xfd.callback.evaluate = (e) => {
		...
		switch (params.category) {
			case 'afd':
				// AFD
				params.logpage = `福瑞百科_talk:存废讨论/记录/${date.format('YYYY/MM/DD', 'utc')}`;
			...
			case 'ffd': {
				...
				wikiPage.setCallbackParameters(params);
				wikiPage.setLookupNonRedirectCreator(true); // Look for author of first non-redirect revision
				wikiPage.lookupCreation(Twinkle.xfd.callbacks.ffd.lookupCreation);
				Morebits.wiki.removeCheckpoint();
				break;
			}
			default:
