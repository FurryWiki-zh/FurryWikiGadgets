...
(function twinkleprotect() {
	...
	Twinkle.protect.callback = () => {
		...
		Window.addFooterLink(window.wgULS('保护方针', '保護方針'), 'FWIKI:PROT');
			...
			list: [
				{
					...
					value: 'request',
					tooltip:
						window.wgULS('若您想在FWIKI:RFPP请求保护此页', '若您想在FWIKI:RFPP請求保護此頁') +
	...
	Twinkle.protect.protectionPresetsInfo = {
		...
		'pp-template': {
			...
			move: 'templateeditor',
			expiry: 'infinity',
			reason: window.wgULS('[[FWIKI:HRT|高风险模板]]', '[[FWIKI:HRT|高風險模板]]'),
			template: 'noop',
		},
		'pp-vandalism': {
			edit: 'sysop',
	...
	Twinkle.protect.callback.evaluate = (e) => {
		...
		switch (input.actiontype) {
			...
			case 'request': {
				...
				Morebits.simpleWindow.setButtonsEnabled(false);
				Morebits.status.init(form);
				const rppName = '福瑞百科_talk:页面保护请求';
	...
	Twinkle.protect.callbacks = {
		...
		fileRequest: (rppPage) => {
			...
			if (text.length === originalTextLength) {
				...
				linknode.append(document.createTextNode(window.wgULS('如何修复RFPP', '如何修復RFPP')));
				statusElement.error([
					window.wgULS(
						'无法在FWIKI:RFPP上找到相关定位点标记，要修复此问题，请参见',
						'無法在FWIKI:RFPP上找到相關定位點標記，要修復此問題，請參見'
					),
					linknode,
					'。',
				]);
		...
		closeRequest: (rppPage) => {
			...
			if (sections.length !== 2) {
				...
				linknode2.append(document.createTextNode('如何修复RFPP'));
				statusElement.error([
					window.wgULS(
						'无法在FWIKI:RFPP上找到相关定位点标记，要修复此问题，请参见',
						'無法在FWIKI:RFPP上找到相關定位點標記，要修復此問題，請參見'
