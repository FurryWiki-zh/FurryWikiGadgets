...
(function twinkleclose() {
	...
	Twinkle.close = () => {
		if (
			...
			!/^福瑞百科([_ ]talk)?:存废讨论\/记录(?:\/\d+){3}$/.test(mw.config.get('wgPageName'))
		) {
			return;
		}
	...
	Twinkle.close.addLinks = () => {
		...
		delLink.append(spanTag('Red', window.wgULS('关闭讨论', '關閉討論')));
		delLink.append(spanTag('Black', ']'));
		...
		if (noop || messageData.action === 'noop') {
			...
			switch (messageData.action) {
				case 'del':
					...
					break;
				case 'keep': {
					const wikiPage = new Morebits.wiki.page(params.title, '移除存废讨论模板');
					wikiPage.setCallbackParameters(params);
					wikiPage.load(Twinkle.close.callbacks.keep);
					break;
				}
				default:
	...
	Twinkle.close.callbacks = {
		del: (params) => {
			...
			if (params.redirects) {
				...
				wikiApi = new Morebits.wiki.api('正在获取重定向', query, Twinkle.close.callbacks.deleteRedirectsMain);
			...
			if (params.talkPage) {
				...
				if (pageTitle && pageTitle.namespace % 2 === 0 && pageTitle.namespace !== 2) {
					...
					wikiApi.params = params;
					wikiApi.apiobj.params.talkPage = pageTitle.toText();
					wikiApi.post();
				}
			}
		...
		deleteRedirectsMain: (apiobj) => {
			...
			redirectDeleter.run((pageName) => {
				...
				wikiPage.setEditSummary(
					`[[FWIKI:CSD#G5|G5]]：${window.wgULS('指向已删页面“', '指向已刪頁面「')}${apiobj.params.title}${window.wgULS(
						'”的重定向',
						'」的重新導向'
					)}`
				);
		...
		deleteTalk: (apiobj) => {
			...
			page.setEditSummary(
				`[[FWIKI:CSD#G5|G5]]：${window.wgULS('已删页面“', '已刪頁面「')}${apiobj.params.title}${window.wgULS(
					'”的[[Help:讨论页|讨论页]]',
					'」的[[Help:討論頁|討論頁]]'
				)}`
			);
		...
		talkend: (params) => {
			const wikiPage = new Morebits.wiki.page(mw.config.get('wgPageName'), '关闭讨论');
			wikiPage.setCallbackParameters(params);
			wikiPage.setPageSection(params.section);
			wikiPage.load(Twinkle.close.callbacks.saveTalk);
		},
		saveTalk: (pageobj) => {
			const statelem = pageobj.getStatusElement();
