...
(function twinklecopyvio() {
	...
	Twinkle.copyvio.callbacks = {
		tryTagging: (pageobj) => {
			...
			if (text.includes('{{Copyvio|')) {
				...
				const wikiPage = new Morebits.wiki.page(mw.config.get('wgPageName'));
				wikiPage.setCallbackParameters(pageobj.getCallbackParameters());
				wikiPage.lookupCreation(Twinkle.copyvio.callbacks.main);
			}
		},
		main: (pageobj) => {
			...
			// Adding discussion
			const wikiPage = new Morebits.wiki.page(params.logpage, window.wgULS('加入侵权记录项', '加入侵權記錄項'));
			wikiPage.setFollowRedirect(true);
			wikiPage.setCallbackParameters(params);
			wikiPage.load(Twinkle.copyvio.callbacks.copyvioList);
	...
	Twinkle.copyvio.callback.evaluate = (e) => {
		...
		wikiPage.setCallbackParameters(params);
		wikiPage.load(Twinkle.copyvio.callbacks.tryTagging);
