...
(function twinkleunlink() {
	 ...
	 * Active on: Non-special pages, except 福瑞百科:沙盒
	 */
	Twinkle.unlink = () => {
		if (
	...
	Twinkle.unlink.callback = (presetReason) => {
		...
		wikiApi.statelem.status(window.wgULS('加载中……', '載入中……'));
	...
	Twinkle.unlink.callback.evaluate = (event) => {
		...
		unlinker.run((pageName) => {
			...
			wikiPage.setBotEdit(true); // unlink considered a floody operation
			wikiPage.setCallbackParameters({
				doBacklinks: input.backlinks.includes(pageName),
				doImageusage: input.imageusage.includes(pageName),
				...params,
			});
			wikiPage.load(Twinkle.unlink.callbacks.unlinkBacklinks);
		});
	};
	Twinkle.unlink.callbacks = {
		...
		unlinkBacklinks: (pageobj) => {
			...
			if (params.doImageusage) {
				text = wikiPage.commentOutImage(mw.config.get('wgTitle'), window.wgULS('注释', '注釋')).getText();
			...
			if (params.doBacklinks) {
				...
				text = wikiPage.removeTemplate(mw.config.get('wgTitle')).getText();
				// did we actually make any changes?
				if (text === oldtext) {
