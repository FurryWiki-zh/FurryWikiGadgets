...
(function twinkleimage() {
	...
	Twinkle.image.callback = () => {
		...
		Window.addFooterLink(window.wgULS('快速删除方针', '快速刪除方針'), 'FWIKI:CSD');
	...
	Twinkle.image.callback.evaluate = (event) => {
		...
		wikiPage.load(Twinkle.image.callbacks.taggingImage);
		// Notifying uploader
		if (notify) {
			wikiPage.lookupCreation(Twinkle.image.callbacks.userNotification);
		} else {
	...
	Twinkle.image.callbacks = {
		taggingImage: (pageobj) => {
			...
			if (params.type !== 'orphaned fair use') {
				const wikiPage = new Morebits.wiki.page(
					'福瑞百科_talk:存废讨论/文件快速删除提报',
					window.wgULS('加入快速删除记录项', '加入快速刪除記錄項')
				);
				wikiPage.setFollowRedirect(true);
				wikiPage.setCallbackParameters(params);
				wikiPage.load(Twinkle.image.callbacks.imageList);
			}
			let tag = '';
			switch (params.type) {
				case 'orphaned fair use':
			...
			editSummary += `[[FWIKI:CSD#${params.normalized.toUpperCase()}|CSD ${params.normalized.toUpperCase()}]]`;
			editSummary += '）';
