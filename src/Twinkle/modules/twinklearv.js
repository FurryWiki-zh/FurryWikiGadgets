...
(function twinklearv() {
	...
	Twinkle.arv.callback = (uid, isIP) => {
		...
		categories.append({
			type: 'option',
			label: window.wgULS('破坏（FWIKI:VIP）', '破壞（FWIKI:VIP）'),
			value: 'aiv',
		});
		categories.append({
			type: 'option',
			label: window.wgULS('编辑争议（FWIKI:EWIP）', '編輯爭議（FWIKI:EWIP）'),
			value: 'ewip',
		});
		categories.append({
			type: 'option',
			label: window.wgULS('用户名（FWIKI:UAA）', '使用者名稱（FWIKI:UAA）'),
			value: 'username',
			disabled: mw.util.isIPAddress(uid),
		});
		categories.append({
			type: 'option',
			label: window.wgULS('傀儡调查（FWIKI:SPI）', '傀儡調查（FWIKI:SPI）'),
			value: 'spi',
			disabled: mw.util.isIPAddress(uid),
		});
		form.append({
			...
			case 'username':
				...
				work_area.append({
					...
					tooltip: window.wgULS(
						'福瑞百科不允许使用带有误导性、宣传性、侮辱性或破坏性的用户名。此外，使用域名及邮箱地址的用户名亦被禁止。这些准则俱应应用至用户名及签名。在其他语言中不当的用户名或通过错拼、替代、暗示、拆字或任何间接方法达成的非妥当用户名同样视为违规。',
						'福瑞百科不允許使用帶有誤導性、宣傳性、侮辱性或破壞性的使用者名稱。此外，使用域名及電子信箱位址的使用者名稱亦被禁止。這些準則俱應應用至使用者名稱及簽名。在其他語言中不當的使用者名稱或通過錯拼、替代、暗示、拆字或任何間接方法達成的非妥當使用者名稱同樣視為違規。'
					),
				});
				work_area.append({
					type: 'checkbox',
					...
					list: [
						...
						},
						{
							...
							tooltip: window.wgULS(
								'宣传性用户名会于福瑞百科上起推销一个组群或一间公司的作用。',
								'宣傳性使用者名稱會於福瑞百科上起推銷一個群組或一間公司的作用。'
							),
						},
						{
						...
						},
						{
							...
							tooltip: window.wgULS(
								'破坏性用户名包括人身攻击、伪冒他人或其他一切有着清晰可见的破坏福瑞百科意图的用户名。',
								'破壞性使用者名稱包括人身攻擊、偽冒他人或其他一切有著清晰可見的破壞福瑞百科意圖的使用者名稱。'
							),
						},
					],
				});
	...
	Twinkle.arv.callback.preview = (form) => {
		...
		switch (input.category) {
			case 'vip':
				title = '福瑞百科_talk:报告当前破坏';
				break;
			case 'ewip':
				title = '福瑞百科_talk:管理员告示板';
				break;
			case 'username':
				title = '福瑞百科_talk:管理员告示板';
				break;
	...
	Twinkle.arv.callback.evaluate = (e) => {
		...
		switch (input.category) {
			...
			case 'ewip': {
				...
				Morebits.wiki.actionCompleted.redirect = '福瑞百科_talk:管理员告示板';
				...
				ewipPage.load(() => {
					...
					const $ewipLink = '<a rel="noopener" target="_blank" href="/wiki/FWIKI:EWIP">FWIKI:EWIP</a>';
			...
			case 'username': {
				...
				uaaPage.load(() => {
					...
					if (
						...
						const $uaaLink = '<a rel="noopener" target="_blank" href="/wiki/FWIKI:UAA">FWIKI:UAA</a>';
						Morebits.status.printUserText(
							reason[1],
							window.wgULS(
			...
			case 'spi': {
				...
				Morebits.simpleWindow.setButtonsEnabled(false);
				Morebits.status.init(form);
				const reportpage = '福瑞百科_talk:管理员告示板';
			...
			default: {
				...
				Morebits.simpleWindow.setButtonsEnabled(false);
				Morebits.status.init(form);
				Morebits.wiki.actionCompleted.redirect = '福瑞百科_talk:报告当前破坏';
				Morebits.wiki.actionCompleted.notice = window.wgULS('报告完成', '報告完成');
				...
				aivPage.load(() => {
					...
					const $aivLink = '<a rel="noopener" target="_blank" href="/wiki/FWIKI:VIP">FWIKI:VIP</a>';
					// check if user has already been reported
					if (
						new RegExp(
							`===\\s*\\{\\{\\s*(?:[Vv]andal)\\s*\\|\\s*(?:1=)?\\s*${Morebits.string.escapeRegExp(
