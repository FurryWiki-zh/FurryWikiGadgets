...
(function twinklespeedy() {
	...
	Twinkle.speedy.initDialog = (callbackfunc) => {
		...
		dialog.addFooterLink(window.wgULS('快速删除方针', '快速刪除方針'), 'FWIKI:CSD');
			...
			const message = [
				...
				$('<a>')
					.attr({
						...
						href: mw.util.getUrl('FWIKI:SPECIALSD'),
					})
		...
		sysop: {
			...
			deleteTalk: (params) => {
				...
				if (
					...
					talkpage.setEditSummary(
						`[[FWIKI:CSD#G9|G9]]: 孤立页面：已删除页面“${Morebits.pageNameNorm}”的讨论页`
					);
			...
			deleteRedirectsMain: (apiobj) => {
				...
				$snapshot.each((_key, value) => {
					...
					page.setEditSummary(`[[FWIKI:CSD#G9|G9]]: 孤立页面：重定向到已删除页面“${Morebits.pageNameNorm}”`);
		...
		user: {
			main: (pageobj) => {
				...
				if (mw.config.get('wgNamespaceNumber') === 6) {
					...
					text = text.replace(
						/\{\{(mtc|(copy |move )?to ?(share|commons)|move to (wikimedia commons)|copy to (wikimedia commons))[^}]*\}\}/gi,
						''
					);
				}
				...
				if (params.normalizeds.length > 1) {
					...
					for (const norm of params.normalizeds) {
						if (norm !== 'db') {
							editsummary += `[[FWIKI:CSD#${norm.toUpperCase()}|CSD ${norm.toUpperCase()}]]、`;
						}
					}
					...
					editsummary =
						window.wgULS('请求[[FWIKI:CSD|快速删除]]：', '請求[[FWIKI:CSD|快速刪除]]：') +
						params.templateParams[0]['1'];
				} else {
					editsummary = `${window.wgULS(
						'请求快速删除',
						'請求快速刪除'
					)}（[[FWIKI:CSD#${params.normalizeds[0].toUpperCase()}|CSD ${params.normalizeds[0].toUpperCase()}]]）`;
				}
				// Blank attack pages
				if (params.blank) {
					text = code;
			...
			addToLog: (params, initialContrib) => {
				...
				usl.initialText = `这是该用户使用[[H:TW|Twinkle]]的速删模块做出的[[FWIKI:CSD|快速删除]]提名列表。\n\n若您不再想保留此日志，请在[[${Twinkle.getPref(
					'configPage'
				)}|参数设置]]中关掉，并使用[[FWIKI:CSD#O1|CSD O1]]提交快速删除。${
					Morebits.userIsSysop ? '\n\n此日志并不记录用Twinkle直接执行的删除。' : ''
				}`;
				let appendText = `# [[:${Morebits.pageNameNorm}]]：`;
				if (params.fromDI) {
					appendText += `图版[[FWIKI:CSD#${params.normalized.toUpperCase()}|CSD ${params.normalized.toUpperCase()}]]（{{tl|${
						params.templatename
					}}}）`;
				} else if (params.normalizeds.length > 1) {
					appendText += '多个理由（';
					for (const norm of params.normalizeds) {
						appendText += `[[FWIKI:CSD#${norm.toUpperCase()}|${norm.toUpperCase()}]]、`;
					}
					...
					appendText += `[[FWIKI:CSD#${params.normalizeds[0].toUpperCase()}|CSD ${params.normalizeds[0].toUpperCase()}]]`;
				}
				if (params.requestsalt) {
	...
	Twinkle.speedy.callback.evaluateUser = (e) => {
		...
		const wikiPage = new Morebits.wiki.page(mw.config.get('wgPageName'), window.wgULS('标记页面', '標記頁面'));
		wikiPage.setCallbackParameters(params);
		wikiPage.load(Twinkle.speedy.callbacks.user.main);
	};
