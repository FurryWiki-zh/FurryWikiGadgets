...
(function twinklebatchdelete() {
	...
	Twinkle.batchdelete.callback = () => {
		...
		const wikiApi = new Morebits.wiki.api(
	...
	Twinkle.batchdelete.deletereasonlist = [
		...
		},
		{
			...
			value: window.wgULS(
				'[[FWIKI:CSD#G6|G6]]：原作者提请删除或清空页面，且页面原作者仅有一人',
				'[[FWIKI:CSD#G6|G6]]：原作者提請刪除或清空頁面，且頁面原作者僅有一人'
			),
		},
		{
			label: window.wgULS(
			...
			value: window.wgULS('[[FWIKI:CSD#G9|G9]]：孤立页面', '[[FWIKI:CSD#G9|G9]]：孤立頁面'),
		},
		{
			label: window.wgULS('R1：不能发挥实际作用的重定向。', 'R1：不能發揮實際作用的重定向。'),
			value: window.wgULS(
				'[[FWIKI:CSD#R1|R1]]：不能发挥实际作用的重定向。',
				'[[FWIKI:CSD#R1|R1]]：不能發揮實際作用的重定向。'
			),
		},
		{
			label: window.wgULS(
			...
			value: window.wgULS(
				'[[FWIKI:CSD#R2|R2]]：名称与导向目标代表事物不一致或不完全一致的重定向。',
				'[[FWIKI:CSD#R2|R2]]：名稱與導向目標代表事物不一致或不完全一致的重定向。'
			),
		},
		{
			label: window.wgULS('F4：可被替代的合理使用文件', 'F4：可被替代的合理使用檔案'),
			value: window.wgULS(
				'[[FWIKI:CSD#F4|F4]]：可被替代的合理使用文件',
				'[[FWIKI:CSD#F4|F4]]：可被替代的合理使用檔案'
			),
		},
		{
			label: window.wgULS(
			...
			value: window.wgULS(
				'[[FWIKI:CSD#F5|F5]]：没有被条目使用的非自由著作权文件、没有填写任何合理使用依据的非自由著作权文件',
				'[[FWIKI:CSD#F5|F5]]：沒有被條目使用的非自由版權檔案、沒有填寫任何合理使用依據的非自由版權檔案'
			),
		},
		{
			label: window.wgULS(
			...
			value: window.wgULS(
				'[[FWIKI:CSD#O1|O1]]：用户请求删除自己的[[Help:用户页|用户页]]或其子页面。',
				'[[FWIKI:CSD#O1|O1]]：使用者請求刪除自己的[[Help:用户页|使用者頁面]]或其子頁面。'
			),
		},
		{
			label: window.wgULS('O2：空的分类（没有条目也没有子分类）', 'O2：空的分類（沒有條目也沒有子分類）'),
			value: window.wgULS(
				'[[FWIKI:CSD#O2|O2]]：空的分类（没有条目也没有子分类）。',
				'[[FWIKI:CSD#O2|O2]]：空的分類（沒有條目也沒有子分類）。'
			),
		},
		{
			label: window.wgULS('O3：废弃草稿', 'O3：廢棄草稿'),
			value: window.wgULS('[[FWIKI:CSD#O3|O3]]：废弃草稿。', '[[FWIKI:CSD#O3|O3]]：廢棄草稿。'),
		},
	];
	...
	Twinkle.batchdelete.callback.evaluate = (event) => {
		...
		pageDeleter.run(
			(pageName) => {
				...
				const wikiPage = new Morebits.wiki.page(
					pageName,
					window.wgULS('正在删除页面 ', '正在刪除頁面 ') + pageName
				);
				wikiPage.setCallbackParameters(params);
				if (delete_page) {
					wikiPage.setEditSummary(`${reason}（批量）`);
					wikiPage.setChangeTags(Twinkle.changeTags);
					wikiPage.suppressProtectWarning();
					wikiPage.deletePage(Twinkle.batchdelete.callbacks.doExtras, pageDeleter.workerFailure);
				} else {
					Twinkle.batchdelete.callbacks.doExtras(wikiPage);
				}
			},
			() => {
				if (delete_subpages) {
	...
	Twinkle.batchdelete.callbacks = {
			...
			if (params.delete_page) {
				...
				if (params.delete_talk) {
					...
					if (pageTitle && pageTitle.namespace % 2 === 0 && pageTitle.namespace !== 2) {
						...
						wikiApi.params = params;
						wikiApi.params.talkPage = pageTitle.toText();
						wikiApi.post();
					}
				}
			}
		...
		deleteRedirectsMain: (apiobj) => {
			...
			redirectDeleter.run((pageName) => {
				const wikiPage = new Morebits.wiki.page(pageName, window.wgULS('正在删除 ', '正在刪除 ') + pageName);
				wikiPage.setEditSummary(
					`[[FWIKI:CSD#G9|G9]]: ${window.wgULS('指向已删页面“', '指向已刪頁面「')}${apiobj.params.page}${window.wgULS(
						'”的重定向',
						'」的重新導向'
					)}`
				);
				wikiPage.setChangeTags(Twinkle.changeTags);
				wikiPage.deletePage(redirectDeleter.workerSuccess, redirectDeleter.workerFailure);
			});
		},
		deleteTalk: (apiobj) => {
			const xml = apiobj.responseXML;
			...
			page.setEditSummary(
				`[[FWIKI:CSD#G9|G9]]: ${window.wgULS('已删页面“', '已刪頁面「')}${apiobj.params.page}${window.wgULS(
					'”的[[Help:讨论页|讨论页]]',
					'」的[[Help:討論頁|討論頁]]'
				)}`
			);
		...
		unlinkBacklinksMain: (apiobj) => {
			...
			unlinker.run((pageName) => {
				...
				wikiPage.setCallbackParameters(params);
				wikiPage.load(Twinkle.batchdelete.callbacks.unlinkBacklinks);
			});
		},
		...
		unlinkImageInstancesMain: (apiobj) => {
			...
			unlinker.run((pageName) => {
				...
				params.unlinker = unlinker;
				wikiPage.setCallbackParameters(params);
				wikiPage.load(Twinkle.batchdelete.callbacks.unlinkImageInstances);
			});
		},
		unlinkImageInstances: (pageobj) => {
			...
			const old_text = text;
			const wikiPage = new Morebits.wikitext.page(text);
			text = wikiPage.commentOutImage(image, window.wgULS('因文件已删，故注解', '因檔案已刪，故註解')).getText();
			Twinkle.batchdelete.unlinkCache[params.title] = text;
			if (text === old_text) {
				pageobj
					.getStatusElement()
