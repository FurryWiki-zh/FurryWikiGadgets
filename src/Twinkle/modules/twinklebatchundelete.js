...
(function twinklebatchundelete() {
	...
	Twinkle.batchundelete.callback.evaluate = (event) => {
		...
		pageUndeleter.run((pageName) => {
			...
			const wikiPage = new Morebits.wiki.page(pageName, window.wgULS('反删除页面', '反刪除頁面') + pageName);
			...
			wikiPage.setMaxRetries(3); // temporary increase from 2 to make batchundelete more likely to succeed
			wikiPage.undeletePage(Twinkle.batchundelete.callbacks.doExtras, pageUndeleter.workerFailure);
		});
	};
		...
		doExtras: (thingWithParameters) => {
			...
			if (params.undel_talk) {
				...
				if (talkpagename !== params.page) {
					...
					wikiApi.params.talkPage = talkpagename;
					wikiApi.post();
				}
			}
		},
