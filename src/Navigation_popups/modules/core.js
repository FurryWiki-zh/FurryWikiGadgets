...
const popups = () => {
	...
	const setupLivePreview = () => {
		...
		Insta.conf = {
			...
			paths: {
				...
				math: '/math/',
				images: 'https://image.furrywiki.org.cn/', // 兽人百科中文站图片路径，或使用 mw.config.get('wgServer') + '/images/'
				images_fallback: '//commons.wikimedia.org/wiki/', // 回退到维基共享资源
			},
			locale: {
		...
		Insta.conf.user.name ||= 'FurryWiki contributors';
	...
	const setOptions = () => {
		...
		newOption('popupRevDelUrl', null); // 移除指向enwiki的链接
	...
	pg.string = {
		...
		defaultpopupReviewedSummary:
			'Accepted by reviewing the [[Special:diff/%s/%s|difference]] between this version and previously accepted version using [[Project:Navigation_popups|popups]]',
		...
		defaultpopupFixRedirsSummary:
			'Redirect bypass from [[%s]] to [[%s]] using [[Project:Navigation_popups|popups]]',
		defaultpopupExtendedRevertSummary:
			'Revert to revision dated %s by %s, oldid %s using [[Project:Navigation_popups|popups]]',
		defaultpopupRevertToPreviousSummary:
			'Revert to the revision prior to revision %s using [[Project:Navigation_popups|popups]]',
		...
		defaultpopupQueriedRevertToPreviousSummary:
			'Revert to the revision prior to revision $1 dated $2 by $3 using [[Project:Navigation_popups|popups]]',
		defaultpopupQueriedRevertSummary:
			'Revert to revision $1 dated $2 by $3 using [[Project:Navigation_popups|popups]]',
